...
//let yeomanImage = require('../images/high1.jpg');
...
class AppComponent extends React.Component {
  ...
  /**
   * 翻转图片
   * index 输入当前执行inverse操作的图片对应的图片信息数组的index值
   * {function}这是一个闭包函数，其中return一个真正待被执行的函数
   */
  inverse(index) {
    return () => {
  ...
  render() {
    ...
    imageDatas.forEach(function(value,index){
      ...
      index} arrange={this.state.imgsArrangeArr[index]} key={index} inverse={this.inverse(index)} center={this.center(index)}/>);
